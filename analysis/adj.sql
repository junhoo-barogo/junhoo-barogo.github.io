...
--  // 기사 차감/충전 금액 합계
    ...
    v_WK_FEE_BR_SUM     NUMBER              := 0;       -- 본사귀속정산-기사의 배달대행수수료 합계(수행지사수익)
    v_WK_FEE_BR_AMT     NUMBER              := 0;       -- 본사귀속정산-기사의 배달대행수수료 공급가액(수행지사수익)
    v_WK_FEE_BR_VAT     NUMBER              := 0;       -- 본사귀속정산-기사의 배달대행수수료 부가세액(수행지사수익)
...
--  // 기사 복수건수 전환 정보
    CURSOR CSR_WK_C_PAY_INFO (c_WK_CODE VARCHAR2, c_DUP_ORDER_CNT NUMBER) IS
        ...
        FROM    ALD_GRP_WK_INFO WK, ALD_GRP_WK_DUP_CNT_DETAIL CD
        WHERE   WK.WK_CGRP_NO = CD.WK_CGRP_NO
...
--  // 배차지사 정보
    CURSOR CSR_BR_INFO_C (c_BR_CODE VARCHAR2) IS
        SELECT  B.DVRY_FEE_FIX, B.DVRY_FEE_VAT_YN, B.DIRECT_YN, B.WK_TAX_RATE, B.WK_TAX_YN, B.BR_VAT_YN, B.BR_DVRY_PAY_FLAG
...
--  // 배차기사 복수건수 전환 정보
    ...
    FETCH   CSR_WK_C_PAY_INFO INTO v_DUP_ORDER_CNT;
...
-- @@@@@@@@@@@@@기사지원금 설정(B2B가 아닌경우 발주지사부담) ==> 설정사항(MATCH_YN)으로 변경
    IF (v_MATCH_YN = 'Y' OR v_B2B_YN = 'N') AND vi_SUPP_AMT > 0 THEN
        ...
        v_DVRY_ADJ_AMT := GREATEST(v_DVRY_ADJ_AMT - v_SUPP_TOT,0); -- 배달대행료 발ㅈ
...
-- @@@@@@@@@@@@@ 정산수수료 할당부분 발주가맹점(본사귀속정산)이고 수행허브(본사귀속정산)인 경우
...
--@@@@@@@@@@@@@@@@@    -- // 가맹점배달대행수수료;
        ...
        --        v_DVRY_ST_PAY_2 := 0;
        --    END IF; 
        IF v_DVRY_ST_PAY_1 > 0 AND v_DVRY_ST_PAY_2 > 0.00 THEN -- 가맹점배달대행수수료;
...
-- // @@@@@@@@@@ 기사의 배달대행수수료
    ...
    ELSIF v_WK_FEE_RATE_SUM > 0.00 THEN
        v_WK_FEE_MSG := '정률, ';
    END IF;
    v_WK_FEE_SUM := (v_WK_FEE_FIX_SUM + v_WK_FEE_RATE_SUM);        -- 기사배달대행수수료총액
...
-- // @@@@@@@@@@ B2B 수행지사 기사배달대행수수료 허브수익처리
    IF v_HQ_BL_YN = 'Y' AND (v_DVRY_ADJ_AMT - v_WK_FEE_SUM) >= 0 THEN -- 부가세/원천세 본사귀속 여부
        v_WK_FEE_BR_AMT := v_WK_FEE_SUM;
        v_WK_FEE_BR_VAT := (v_WK_FEE_SUM * 10/100);
...
--      1-2. 라이더-배달대행수수료-충전      => C3
        IF (v_WK_FEE_BR_AMT + v_WK_FEE_BR_VAT) > 0 THEN
            INSERT INTO ALD_ADJ_BR_CASH_LOG
                        (HD_CODE, BR_CODE, BR_CASH_TYPE_CD
                        , ADD_CASH, ADD_AMT, ADD_VAT, ORD_NO, BR_CASH_MEMO, IN_USR_ID)
            VALUES      (vi_CTH_HD_CODE, vi_CTH_BR_CODE, 'C3'
                        , (v_WK_FEE_BR_AMT + v_WK_FEE_BR_VAT), v_WK_FEE_BR_AMT, v_WK_FEE_BR_VAT
...
--  // 정산테이블 입력
    INSERT INTO ALD_A01_CHARGE
                ...
                , CARD_APPR_NUM, VAN_TERM_KIND, VAT_RATE, C_BR_DIRECT_YN, WK_TAX_RATE, WK_TAX_YN
    ...
    VALUES      (vi_ORD_NO, vi_ORD_TYPE_CD
                ...
                , vi_CARD_APPR_NUM, v_VAN_TERM_KIND, v_VAT_RATE, v_DIRECT_YN, v_WK_TAX_RATE, v_WK_TAX_YN
